...
function solve_iterative!(m)
    ...
    while true
        if (m.logs[:n_iter] >= 1) && check_gap!(m)
            return :Optimal
        end
        ...
        elseif check_gap!(m)
            ...
            # Solve was an optimal solve, integer solution has repeated and no violated cuts were added, haven't converged: fail
            return :NoViolatedCuts
        end
    end
end
