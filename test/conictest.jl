...
function runconictests(branch_cut, mip_solver, conic_solver)
    ...
    #                                     x^2 <= 9)
    #
    #                 Convex.solve!(problem, PajaritoSolver(solver_con=conic_solver))
    #                 @fact problem.optval --> roughly(9.0, TOL)
    #         end
    #     end
    # end
    ...
    #                                     x^2 <= 9)
    #
    #                 Convex.solve!(problem, PajaritoSolver(solver_mip=mip_solver))
    #                 @fact problem.optval --> roughly(9.0, TOL)
    #         end
    #     end
    # end
    ...
    facts("\n\n\n\nInfeasible conic problem\n\n") do
        context("With $algorithm, $(typeof(mip_solver)) and $(typeof(conic_solver))") do
            ...
            Convex.solve!(problem, PajaritoSolver(branch_cut=branch_cut, solver_mip=mip_solver, solver_con=conic_solver))
    ...
    facts("\n\n\n\nContinuous problem\n\n") do
        context("With $algorithm, $(typeof(mip_solver)) and $(typeof(conic_solver))") do
           ...
           @fact_throws ErrorException Convex.solve!(problem, PajaritoSolver(branch_cut=branch_cut, solver_mip=mip_solver, solver_con=conic_solver))
       end
    end
    ...
    facts("\n\n\n\nNo SOC disaggregation test\n\n") do
        context("With $algorithm, $(typeof(mip_solver)) and $(typeof(conic_solver))") do
            ...
            Convex.solve!(problem, PajaritoSolver(branch_cut=branch_cut, disagg=false, solver_mip=mip_solver, solver_con=conic_solver))
